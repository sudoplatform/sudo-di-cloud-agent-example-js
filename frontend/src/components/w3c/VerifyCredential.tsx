...
export const VerifyCredential = (props: Props): ReactElement => {
  ...
  /**
   * Event handler.
   * Contact the backend to verify a credential when the "Request Proof of Residency" button is clicked.
   */
  const handleOnClickRequestProofPresentation = async () => {
    ...
    const responseData = await HttpToBackend.sendW3cProofRequest(connectionId, issuerDid);
  ...
  const pollProofPresentationExchangeState = async () => {
    ...
    while (!isExchangeComplete) {
      ...
      setAttributeGivenName(responseData.revealedAttributes.givenName ?? '');
      setAttributeFamilyName(responseData.revealedAttributes.familyName ?? '');
      await sleep(1000);
    }
    ...
    <div id="verifyCredential">
      <table>
        <tbody>
          ...
          <tr className={focusOnStep(step, ExampleStep.PresentProof)}>
            ...
            <td>
              <span id="presentProofInstructions">
                Use your chosen holder's wallet app to present the issued "Resident" credential for verification.
              </span>
            </td>
          </tr>
          <tr className={focusOnStep(step, ExampleStep.PresentProof)}>
          ...
          <tr className={focusOnStep(step, ExampleStep.PresentProof)}>
            <td></td>
            <td colSpan={2}>Data from the presented "Resident" credential:</td>
            <td></td>
          </tr>
          <tr className={focusOnStep(step, ExampleStep.PresentProof)}>
            <td></td>
            <td className="credential-attribute">given name:</td>
            <td>
          ...
          <tr className={focusOnStep(step, ExampleStep.PresentProof)}>
            <td></td>
            <td className="credential-attribute">family name:</td>
            <td>
